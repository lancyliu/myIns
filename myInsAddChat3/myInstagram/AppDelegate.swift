...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, MessagingDelegate{
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any]) {
        ...
        let cont = newSb.instantiateViewController(withIdentifier: "ChatViewController") as? ChatViewController
        cont?.receiverId = uid
        cont?.rootView = "AppDelegate"
        mainTabCont?.selectedIndex = 4
        ...
        if UIApplication.shared.applicationState == .background{
            ...
            //
            TWMessageBarManager.sharedInstance().showMessage(withTitle: "New Message", description: "You have a new message, go to chat page to see it", type: .info)
        }
